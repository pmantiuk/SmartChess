...
def get():
    ...
    # to indicate the current last line of stdout
    stx=""
...
def bmove(fmove):
    """ assume we get a command of the form ma1a2 from the board"""
    fmove=fmove
    ...
    # elif valid make the move and send Fen to board
        ...
        # retrieve the remote board's move.
        text = getAdafruit()
        print (text)
        smove = text
        hint = "xxxx"
        ...
        stx = smove+hint
        maxchess.printBoard()
        print ("Remote player's move: " +smove)
        smove ="m"+smove
...
# get initial settings (such as level)
...
# get initial settings (such as move time)
...
while True:
    ...
    # decide which function to call based on the first letter of txt
    fmove = fmove
    if code == 'm':
        fmove = bmove(fmove)
    elif code == 'n':
